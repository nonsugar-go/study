...
4. インタフェースの一覧から各行をダブルクリックして編集 (eth2 の例)
   1. コメントの入力: Internal1 **適切なコメントを入力**
   2. General > トポロジ > `[Modify...]` をクリック
      - 対象先: 上書き
        - このネットワーク (内部): (X)
      ...
      - セキュリティ ゾーン: トポロジに基づく
      - アンチスプーフィング
        - インタフェース トポロジに基づくアンチスプーフィングを行う
          - アンチスプーフィング アクションを以下に設定: Prevent
          - スプーフ追跡: Log
5. 同様に全てのインタフェースの設定をする
   最終的に下記の状態にする
   | 名前 | トポロジ           | バーチャル IP     | CPGW1A           | CPGW1B           | コメント   |
   |------|-------------------|------------------|------------------|------------------|-----------|
   | eth0 | This network      | Private          | 192.168.1.42/24  | 192.168.1.43/24  | Mgmt      |
   | eth1 | This network      | Sync             | 192.168.255.1/24 | 192.168.255.2/24 | Sync      |
   | eth2 | This network      | 192.168.101.1/24 | 192.168.101.2/24 | 192.168.101.3/24 | Internal1 |
   | eth3 | This network      | 192.168.102.1/24 | 192.168.102.2/24 | 192.168.102.3/24 | Internal2 |
...
8. 同様に全てのインタフェースの設定をする
   ...
   | 名前 | トポロジ           | IP               | コメント   |
   |------|-------------------|------------------|-----------|
   | eth0 | This network      | 192.168.1.44/24  | Mgmt      |
   | eth1 | This network      | 192.168.201.2/24 | Internal  |
   | eth2 | External          | 192.0.2.65/26    | External  |

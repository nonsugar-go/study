...
```
add arp proxy ipv4-address 192.0.2.11 interface eth4 real-ipv4-address 192.0.2.2
save config
```
...
```
add arp proxy ipv4-address 192.0.2.11 interface eth4 real-ipv4-address 192.0.2.3
save config
```

### ホスト・サービス・アクセス コントロール・NAT

```
add host name WebSrvPrivate ip-address 192.168.111.11 color "orange"
add host name WebSrvPublic ip-address 192.0.2.11 color "red"
add service-tcp name "http_5080" port 5080 color "sea green"

add access-rule layer "Network" position.bottom "New rules" name "Outside to WebSrv Accept" source.1 "ExternalZone" destination.1 "WebSrvPublic" service.1 "http" action "Accept" track.type "Log"

add nat-rule package "standard" position "bottom" name "WebSrv_Out" method "static" original-source "WebSrvPrivate" original-destination "Any" original-service "Any" translated-source "WebSrvPublic" translated-destination "Original" translated-service "Original" install-on.1 "CPGW1"

add nat-rule package "standard" position "bottom" name "WebSrv_In" method "static" original-source "Any" original-destination "WebSrvPublic" original-service "http" translated-source "Original" translated-destination "WebSrvPrivate" translated-service "http_5080" install-on.1 "CPGW1"
```

**Manual Lower Rules**
|名前      |元の発信元    |元の宛先     |元のサービス|変換後の発信元  |変換後の宛先              |変換後のサービス|インストール|
|----------|-------------|------------|-----------|---------------|-------------------------|--------------|-----------|
|WebSrv_Out|WebSrvPrivate|Any         |Any        |S) WebSrvPublic|＝ Oritinal              |＝ Oritinal   |CPGW1      |
|WebSrv_In |Any          |WevSrvPublic|http       |＝ Oritinal    |S) WebSrvPrivate         |http_5080     |CPGW1      |
